/*****************************************************************
            This pll.c generates half of the input frequency 
 *****************************************************************/
...
pll_state_def *pll_init(int k, int D, float f, float w_0, int T, int S)
{
    ...
    s->y = 0;       // Output value
    s->accumhalf = 0;   //Accumulator,used for the frequency in the sin table.
...
void pll(pll_state_def *s, const float x_in[], float y_out[])
{
    ...
    {   
        ...
        s->accumhalf = s->accumhalf + 1/2 * (s->f - (s->k * s->vc / (2 * PI))); //calculate half frequency
        s->accumhalf = s->accumhalf - floor(s->accumhalf);
        
        // calculate output using sine table
        // objective is y_out[n + 1] = sin(2 * PI * acumm[n + 1])
        y_out[i] = s->sine_table[floor(s->accumhalf * s->S)]; //Where S = 1024
