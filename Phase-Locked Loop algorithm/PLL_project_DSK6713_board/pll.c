...
pll_state_def *pll_init(int k, int D, float f, float w_0, int T, int S)
{
    ...
    s->y = 0;       // Output value
    s->accum = 0;   //Accumulator,used for the frequency in the sin table.
...
void pll(pll_state_def *s, const float x_in[], float y_out[])
{
    ...
    {   
        ...
        s->accum = s->accum + s->f - (s->k * s->vc / (2 * PI)); //calculate same frequency
        s->accum = s->accum - floor(s->accum);
        
        // calculate output using sine table
        // objective is y_out[n + 1] = sin(2 * PI * acumm[n + 1])
        y_out[i] = s->sine_table[floor(s->accum * s->S)]; //Where S = 1024
    ...
    //Update the amplitude value for next normalization.
